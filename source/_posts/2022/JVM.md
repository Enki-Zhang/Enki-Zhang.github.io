...
![asset_img](JVM/2022-10-26-16-32-27.png)
...
一款软件 相当于"虚拟的计算机"，可以划分为：

- 系统虚拟机
  - visual box, VMare 等安装虚拟机的软件，是对物理计算机的仿真
- 程序虚拟机
  - java 虚拟机 专门为了执行单个计算机程序而设计 。jvm 可以执行符合 jvm 规范不同语言的字节码文件。具有独立的运行机制 ，使用 jvm 平台的各种语言（JavaScript 等等）可以共享 java 虚拟机的跨平台性，垃圾回收以及即时编译。所有的 java 程序都运行在 java 虚拟机中。

## jvm 的位置

jvm 运行在 os 之上，与硬件没有直接的交互。由内到位分别是硬件->os->jvm->字节码->用户

## JVM 的整体结构

hotspot vm 是主流代表 采用解释器和即时编译器并存的架构。其内存结构图如下：
![asset_img](JVM/image.png.png)

> class files 到类装载器中间经过的编译器的前端
> 方法区和堆：多线程共享
...
![asset_img](JVM/2022-10-25-15-52-40.png)
首先 javac 将源码编译为字节码，经过词法分析、语法分析、语义分析，编译原理中这部分的编译称为**前端编译**。接下来无需编译直接逐条将字节码解释执行（确保响应时间），在解释执行的过程中，虚拟机同时对程序运行的信息进行收集，在这些信息的基础上，编译器会逐渐发挥作用，它会进行后端编译——把字节码编译成机器码，但不是所有的代码都会被编译，只有被 JVM 认定为的热点代码，才可能被编译。也就是说部分代码在一定时间内多次调用超过 JVM 设置的阈值后，JIT 编译器将这段代码编译为机器指令，并存入缓存中，当下次遇见时从缓存中直接读取机器码从而提高机器运行的效率。
![asset_img](JVM/2022-10-25-15-34-23.png)
...
2. 避开了寄存器的分配难题：使用零地址指令方式分配；(零地址只需要操作栈顶元素，不需要知道元素地址)

3. 指令流中的指令大部分是零地址指令，其执行过程依赖于操作栈。指令集更小(相比于寄存器方法，完成同样的操作指令就需要更多【栈的指令集比寄存机的小】)，编译器容易实现。

4. 不需要硬件支持，可移植性更好，更好实现跨平台；

- 基于寄存器架构的特点 1.典型的应用是 X86 的二进制指令集：比如传统的 pc 以及 andriod 的 Davlik 虚拟机；

2. 指令集架构则完全依赖于硬件（cpu），可移植性差；

3. 性能优秀和执行更高效；

4. 花费更少的指令去完成一项操作。

5. 在大部分情况下，基于寄存器架构的指令集往往都以一地址指令、二地址指令和三地址指令为主，而基于栈式架构的指令集确是以零地址指令为主

> java 的跨平台需要，尽量不依赖于物理硬件，java 指令都是根据栈来设计的。栈架构的优点：跨平台、指令集小、编译器容易实现。缺点：性能下降，不如寄存器方式

## JVM 的生命周期

- 启动：Java 虚拟机的启动是通过引导类加载器（bootstrap class loader)创建一个初始类（initial class)来完成的，这个类是由虚拟机的具体实现指定的。
- 执行：一个运行中的 Java 虚拟机有着一清晰的任务：执行 Java 程序。程序开始执行时他才运行，程序结束时他就停止。
  执行一个所谓的 Java 程序的时候，真正正在执行的是一个叫做 Java 虚拟机的进程。
- 退出：
  • 程序正常执行结束
...
![asset_img](JVM/2022-10-25-21-54-32.png)
作用：

- 类加载器子系统负责从文件系统或者网络中加载.class 文件，.class 文件（可以用任何语言来实现只要符合 JVM 规范）在文件开头有特定的文件标识。【验证阶段】

- ClassLoader【类加载器】ClassLoader 的具体作用就是将 class 文件加载到 jvm 虚拟机中去，其只负责.class 文件的加载，至于它是否可以运行，则由 Execution Engine 决定

- 加载的类信息存放于一块称为方法区的内存空间。除了类的信息外，方法区中还会存放运行时常量池信息，可能还包括字符串字面量和数字常量（这部分常量信息是 Class 文件中常量池部分的内存映射）

#### classLoader 角色

![asset_img](JVM/2022-10-26-13-47-50.png)
.class 存在于本地硬盘上，由 java 文件解析生成，可以理解为设计师画在纸上的模板，而最终这个模板在执行的时候是要加载到 JVM 当中来根据这个文件实例化出 n 个一模一样的实例

> Car.java —javac 编译为字节码—> Car.class —JVM 的 ClassLoader—>Class<Car>(类的模板)—newInstance()—> Car 的实例对象
> Class clazz = Car.class;
> Car car = clazz.newInstance();

class file 加载到 JVM 中，被称为 DNA 元数据模板，放在方法区
在.class 文件–>JVM–>最终成为元数据模板(Class 类)，此过程就要一个运输工具（类装载器 Class Loader），扮演一个快递员的角色。

#### 类加载过程

这里可以看这个博客[https://blog.csdn.net/m0_37989980/article/details/111201125]
...
定义：一块较小的内存空间：相当于当前线程所执行的字节码的行号指示器。
作用：字节码解释器的工作是通过改变程序计数器的值选取下一条需要执行的字节码指令，是程序控制流的指示器，分支、循环、跳转、异常处理、线程恢复等基础功能都需要依赖这个计数器来完成。
特点：每条线程都有一个独立的程序计数器，且各个计数器之间相互不影响，且占用内存区域中的线程私有的内存即存储独立。

- 对于正在执行的 java 方法，计数器记录的是当前执行的虚拟机字节码指令地址。
- 若是本地 native 方法，则计数器值为空，且此内存区域是唯一一个在《Java 虚拟机规范》中没有规定任何 OutOfMemoryError 情况的区域
...
Java 虚拟机栈：Java Virtual Machine Stacks，**每个线程**运行时所需要的内存，且是线程私有的和相互独立不能互相引用

- 每个方法被执行时，都会在虚拟机栈中创建一个栈帧 stack frame（**一个方法一个栈帧**）【方法被调用直到执行完毕，对应着栈帧在虚拟机栈中的入栈到出栈的过程，理解为每个方法需要的内存为栈帧】
...
- 每个栈由多个栈帧（Frame）组成，对应着每次方法调用时所占用的内存，每个栈帧中存储着：
  ...
  - 操作数栈或表达式栈和其他一些附加信息
    ...
    ![asset_img](JVM/2022-10-27-15-48-02.png)
    ![asset_img](JVM/2022-10-27-16-28-28.png)

  > 栈帧:也叫过程活动记录，是编译器用来实现过程/函数调用的一种数据结构。简言之，栈帧就是利用 EBP（栈帧指针，请注意不是 ESP）寄存器访问局部变量、参数、函数返回地址等的手段。

设置栈内存大小：`-Xss size`: `-Xss 1024k` 栈内存越大，单个线程占用空间就大，一定物理内存中的线程数就小

- 在 JDK 1.4 中默认为 256K，而在 JDK 1.5+ 默认为 1M

虚拟机栈特点：

- 栈内存**不需要进行 GC**，【GC 用在堆上】方法开始执行的时候会进栈，方法调用后自动弹栈，相当于清空了数据

- 栈内存分配越大越大，可用的线程数越少（内存越大，每个线程拥有的内存越大）

- 方法内的局部变量是否**线程安全**：
  - 如果方法内局部变量没有逃离方法的作用访问【非共享，私有变量】，它是线程安全的（逃逸分析：变量是否可以通过外部进行访问、修改。例如作为变量作为参数，或者变量作为返回值）
  - 如果是局部变量引用了对象，并逃离方法的作用范围【共享变量】，需要考虑线程安全

异常：内存溢出

- 栈帧过多导致栈内存溢出 （超过了栈的容量），会抛出 OutOfMemoryError 异常【递归出口异常，内存溢出】
...
参数值存放从是局部变量表的 index0 开始到-1 结束，基本单位为 slot 变量槽，

- 表是建立在线程的栈上，是线程私有的数据，因此不存在数据安全问题
- 表的容量大小是在**编译期**确定的，保存在方法的 Code 属性的 maximum local variables 数据项中
...
public class LocalVariableTable {
    ...
    public static void main(String[] args) {
        int add = new LocalVariableTable().add(1, 2);
...
// access flags 0x21
public class com/enki/LocalVariableTable {
  ...
  public <init>()V
   L0
    ...
    RETURN
   L1
    LOCALVARIABLE this Lcom/enki/LocalVariableTable; L0 L1 0
    MAXSTACK = 1
    MAXLOCALS = 1
  ...
  public add(II)I
   ...
   L4
    ...
    IRETURN
   L5
    LOCALVARIABLE j J L2 L3 5
    LOCALVARIABLE this Lcom/enki/LocalVariableTable; L0 L5 0
    LOCALVARIABLE x I L0 L5 1
    LOCALVARIABLE y I L0 L5 2
    LOCALVARIABLE i J L1 L5 3
    LOCALVARIABLE z I L4 L5 5
    MAXSTACK = 2
    MAXLOCALS = 7
  ...
  public static main([Ljava/lang/String;)V
   L0
    LINENUMBER 19 L0
    NEW com/enki/LocalVariableTable
    DUP
    INVOKESPECIAL com/enki/LocalVariableTable.<init> ()V
    ICONST_1
    ICONST_2
    INVOKEVIRTUAL com/enki/LocalVariableTable.add (II)I
    ISTORE 1
   L1
    LINENUMBER 20 L1
   ...
   L3
    ...
    LOCALVARIABLE add I L1 L3 1
    MAXSTACK = 3
    MAXLOCALS = 2
}

第七篇 JVM之运行时数据区&lt;3&gt;: 局部变量表
https://blog.51cto.com/u_15127583/4530206
...
- 存放编译期可知的各种基本数据类型（8 种），引用类型（reference），returnAddress 类型【指向了一条字节码指令的地址】的变量
...
> 成员变量
> 类变量：独立于方法之外的变量，用 static 修饰。在 linking 的 prepare 阶段给类变量默认赋值
> 实例变量：独立于方法之外的变量，不过没有 static 修饰.在对象创建过程中会在堆空间中分配实例变量空间并进行默认初始化
> 局部变量：类的方法中的变量，需要进行**显式的复制**，否则编译不通过。
...
- 操作数栈是 jvm 的执行引擎的一个工作区，在一个方法刚开始执行的时候会创建一个新的且操作数栈为空的栈帧

- 且每个操作数栈都有一个明确深度的栈(即数组长度固定)用于存储数值，深度在编译期间就定义好了，保存在方法 code 中的 max_stack 中

- 用于保存计算过程的中间结果，同时作为计算过程中变量临时的存储空间，是执行引擎的一个工作区
- Java 虚拟机的解释引擎 是基于栈的执行引擎，其中的栈指的就是操作数栈
...
解释：被调用的目标方法在编译期无法被确定下来，只能够在程序运行期将方法的符号引用转换为直接引用，这种引用转换的过程具备动态性，称为动态链接。
动态链接是指向运行时常量池（是方法区的一部分）的方法引用，涉及到栈操作已经是类加载完成，这个阶段的解析是**动态绑定**或者晚期绑定
...
![asset_img](JVM/2022-10-28-15-35-55.png)
...
- Tenured 区(老年代-养老区)主要保存生命周期长的对象，一般是一些老的对象，当一些对象在 Young 复制转移一定的次数以后，对象就会被转移到 Tenured 区
...
> 修改虚拟机参数 ![asset_img](JVM/2022-10-27-17-09-13.png)
...
- 符号引用：类、字段、方法、接口等的符号引用【符号引用：在 java 中，一个 java 类将会编译成一个 class 文件。在编译时，java 类并不知道引用类的实际内存地址，因此只能使用符号引用来代替。比如 org.simple.People 类引用 org.simple.Tool 类，在编译时 People 类并不知道 Tool 类的实际内存地址，因此只能使用符号 org.simple.Tool(假设)来表示 Tool 类的地址。而在类装载器装载 People 类时，此时可以通过虚拟机获取 Tool 类 的实际内存地址，因此便可以既将符号 org.simple.Tool 替换为 Tool 类的实际内存地址，及直接引用地址】
- 方法引用/直接引用 指在运行期间使用的引用方式 指向类 方法或字段的直接内存地址
...
- `-XX:TLABWasteTargetPercent`：设置 TLAB 空间所占用 Eden 空间的百分比大小，默认情况下 TLAB 空间的内存非常小，仅占有整个 Eden 空间的 1%
...
手动 GC 测试，VM 参数：`-XX:+PrintGcDetails`
...
> GC 进行扫描时，需要查看每个位置存储的是不是引用类型，如果是，其所引用的对象就不能被回收；如果不是，那就是基本类型，这些肯定是不会引用对象的；这种对 GC 无用的基本类型的数据非常多，每次 GC 都要去扫描，显然是非常浪费时间的。安全点用空间换取时间的方法。
...
> 当线程处于 sleep 或者 blocked 状态时，线程就不能走到安全点，此时需要引入安全区域
...
引用计数算法（Reference Counting）：对每个对象保存一个整型的引用计数器属性，用于记录对象被引用的情况。对于一个对象 A，只要有任何一个对象引用了 A，则 A 的引用计数器就加 1；当引用失效时，引用计数器就减 1；当对象 A 的引用计数器的值为 0，即表示对象 A 不可能再被使用，可进行回收（Java 没有采用）
...
- 虚拟机栈中栈帧中的局部变量表中引用的对象：各个线程被调用的方法中使用到的参数、局部变量等
- 本地方法栈中引用的对象
- 堆中类静态属性引用的对象
- 方法区中的常量引用的对象
...
4. 虚引用（PhantomReference）：也称为幽灵引用或者幻影引用，是所有引用类型中最弱的一个
   ...
   对对象没有半毛钱影响，甚至不能用来取得一个对象的实例。它唯一的用途就是：当被一个虚引用引用的对象被回收时，系统会收到这个对象被回收了的通知。实现类:`PhantomReference`
...
![asset_img](JVM/2022-11-04-13-03-45.png)
步骤：

1. 找到死去的对象
2. 回收

用可达性分析找到死去的对象之前需要枚举 GCRoots 但显然每次枚举都要遍历整个栈是不合理的，就需要使用 oopMap 的数据结构【记录了栈在本地变量到堆上的对象引用关系】。所以只需遍历 oopMap 就可以知道那些对象可以进行回收。
但是如果每有一个对象被回收后，都对 oopMap 进行更新，那会十分频繁且耗时。所以 hotspot 设置了在**安全点**更新 oopMap 的方法,安全点的设置同样十分讲究，不能让 GC 等待时间太长也不能频繁 GC,通常会在长时间执行指令的地方设置安全点：方法调用、循环跳转、异常跳转等。此外，在进行 GCRoots 枚举时，为了保证准确性，一般会进入 Stop The World（STW GC 停顿）。线程进入安全点后开启 STW 进行 GC,让所有线程进入安全点的方式：
...
但是并不是所有线程都能够到达安全点，像处于 sleep 或者 blocked 状态。所以需要引入安全区（安全区域是指在一段代码片段之中，引用关系不会发生变化，因此在这个区域中的任意位置开始 GC 都是安全的）。
当线程来到安全区中时，将自己标示为 safe region 这样 JVM 在 GC 中就不会处理该线程
当这个线程要离开安全区域时，它会检查系统是否在 GC 中，如果不在，它就继续执行，如果在，它就等 GC 结束再继续执行。
...
- `-XX：+UseConcMarkSweepGC`：手动指定使用 CMS 收集器执行内存回收任务
  ...
  开启该参数后会自动将 `-XX:+UseParNewGC` 打开，即：ParNew + CMS + Serial old 的组合
...
- `-XX:ParallelCMSThreads`：设置 CMS 的线程数量
  ...
  - 收集线程占用的 CPU 资源多于 25%，对用户程序影响可能较大；当 CPU 资源比较紧张时，受到 CMS 收集器线程的影响，应用程序的性能在垃圾回收阶段可能会非常糟糕
...
- **分区算法**：
  ...
  - 将整个堆划分成约 2048 个大小相同的独立 Region 块，每个 Region 块大小根据堆空间的实际大小而定，整体被控制在 1MB 到 32 MB 之间且为 2 的 N 次幂，所有 Region 大小相同，在 JVM 生命周期内不会被改变。G1 把堆划分成多个大小相等的独立区域，使得每个小空间可以单独进行垃圾回收
...
- **Young GC**：发生在年轻代的 GC 算法，一般对象（除了巨型对象）都是在 eden region 中分配内存，当所有 eden region 被耗尽无法申请内存时，就会触发一次 Young GC，G1 停止应用程序的执行 STW，把活跃对象放入老年代，垃圾对象回收
  ...
  **回收过程**：
  ...
  4. 复制对象：Eden 区内存段中存活的对象会被复制到 survivor 区，survivor 区内存段中存活的对象如果年龄未达阈值，年龄会加 1，达到阀值会被会被复制到 old 区中空的内存分段，如果 survivor 空间不够，Eden 空间的部分数据会直接晋升到老年代空间
...
JDK7 中的 String：2 个 int 值共占 8 字节，value 对象引用占用 4 字节，对象头 8 字节，对齐后占 24 字节，为 String 对象的浅堆大小，与 value 实际取值无关，无论字符串长度如何，浅堆大小始终是 24 字节
...
1. 创建阶段 (Created)：
2. 应用阶段 (In Use)：对象至少被一个强引用持有着 3.不可见阶段 (Invisible)：程序的执行已经超出了该对象的作用域，不再持有该对象的任何强引用 4.不可达阶段 (Unreachable)：该对象不再被任何强引用所持有，包括 GC Root 的强引用
   5 收集阶段 (Collected)：垃圾回收器对该对象的内存空间重新分配做好准备，该对象如果重写了 finalize() 方法，则会去执行该方法 6.终结阶段 (Finalized)：等待垃圾回收器对该对象空间进行回收，当对象执行完 finalize() 方法后仍然处于不可达状态时进入该阶段 7.对象空间重分配阶段 (De-allocated)：垃圾回收器对该对象的所占用的内存空间进行回收或者再分配
...
时机：执行了 System.exit() 方法，程序正常执行结束，程序在执行过程中遇到了异常或错误而异常终止，由于操作系统出现错误而导致 Java 虚拟机进程终止
...
- 实现程序的动态性，如代码热替换（Hot Swap）、模块热部署（Hot Deployment）
  ...
  IBM 公司主导的 JSR 一 291（OSGiR4.2）实现模块化热部署的关键是它自定义的类加载器机制的实现，每一个程序模块（OSGi 中称为 Bundle）都有一个自己的类加载器，当更换一个 Bundle 时，就把 Bundle 连同类加载器一起换掉以实现代码的热替换，在 OSGi 环境下，类加载器不再双亲委派模型推荐的树状结构，而是进一步发展为更加复杂的网状结构
...
| u2             | interfaces_count    | 接口计数             | 2 个字节 | 1                     |
...
版本：4 个 字节，5 6 两个字节代表的是编译的副版本号 minor_version，而 7 8 两个字节是编译的主版本号 major_version
...
访问标识（access_flag），又叫访问标志、访问标记，该标识用两个字节表示，用于识别一些类或者接口层次的访问信息，包括这个 Class 是类还是接口，是否定义为 public 类型，是否定义为 abstract 类型等
...
- 父类索引用于确定这个类的父类的全限定名，Java 语言不允许多重继承，所以父类索引只有一个，除了 Object 之外，所有的 Java 类都有父类，因此除了 java.lang.Object 外，所有 Java 类的父类索引都不为 0
...
attributes\_ count（属性计数器）：表示当前文件属性表的成员个数
...
- lcmp：比较两个 long 类型值
- fcmpl：比较两个 float 类型值（当遇到 NaN 时，返回-1）
- fcmpg：比较两个 float 类型值（当遇到 NaN 时，返回 1）
- dcmpl：比较两个 double 类型值（当遇到 NaN 时，返回-1）
- dcmpg：比较两个 double 类型值（当遇到 NaN 时，返回 1）
...
- main 线程开始运行，分配栈帧内存：（操作数栈 stack=2，局部变量表 locals=4）
...
对应方法的绑定（分配）机制：早期绑定->静态链接和晚期绑定->动态链接。绑定是一个字段、方法或者类从符号引用被替换为直接引用的过程，**仅发生一次**：

- 早期绑定：被调用的目标方法在编译期可知，且运行期保持不变，将这个方法与所属的类型进行绑定
- 晚期绑定：被调用的目标方法在编译期无法确定，只能在程序运行期根据实际的类型绑定相关的方法
...
- 在于对类型的检查是在**编译期**还是在运行期，满足前者就是静态类型语言，反之则是动态类型语言
...
语法糖：指 Java 编译器把 _.java 源码编译为 _.class 字节码的过程中，自动生成和转换的一些代码，主要是为了减轻程序员的负担
...
性能指标主要是吞吐量、响应时间、QPS、TPS 等、并发用户数等，而这些性能指标又依赖于系统服务器的资源，如 CPU、内存、磁盘 IO、网络 IO 等，对于这些指标数据的收集，通常可以根据 Java 本身的工具或指令进行查询
...
ostid 参数：RMI 注册表中注册的主机名，如果想要远程监控主机上的 java 程序，需要安装 jstatd

---

#### jstat

jstat（JVM Statistics Monitoring Tool）：用于监视 JVM 各种运行状态信息的命令行工具，可以显示本地或者远程虚拟机进程中的类装载、内存、垃圾收集、JIT 编译等运行数据，在没有 GUI 的图形界面，只提供了纯文本控制台环境的服务器上，它是运行期定位虚拟机性能问题的首选工具，常用于检测垃圾回收问题以及内存泄漏问题
...
- option 参数：
  ...
  - -gc：显示与 GC 相关的堆信息，年轻代、老年代、永久代等的容量、已用空间、GC 时间合计等信息
...
- -dump：生成 dump 文件（Java 堆转储快照，二进制文件），-dump:live 只保存堆中的存活对象
