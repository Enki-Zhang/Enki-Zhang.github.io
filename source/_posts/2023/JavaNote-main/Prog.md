...
进程：程序是静止的，进程实体的运行过程就是进程，是系统进行**资源分配的基本单位** 类似于一个应用程序
...
- 并发：在同一时刻，有多个指令在单个 CPU 上交替执行(单核 cpu 一次只能处理一个任务 实际多线程不能真正提高效率，只是不让 cpu 空闲而已)
...
区别在于实现 runable 接口的方法需要丢入到 Thread 方法中
...
| public static void sleep(long time)         | 让当前线程休眠多少毫秒再继续执行<br />**Thread.sleep(0)** : 让操作系统立刻重新进行一次 CPU 竞争 | 源  |
...
- 具体的实现依赖于操作系统的任务调度器
- **会放弃 CPU 资源，锁资源不会释放**
  > wait 是 object 类的方法 将线性暂停 需要其他线程使用 notify()或 notifyAll()来唤醒 wait 的线程
...
**synchronized：对象锁，保证了临界区内代码的原子性**，临界区内的代码对外是不可分割的，不会被线程切换所打断，采用互斥的方式让同一时刻至多只有一个线程能持有对象锁，其它线程获取这个对象锁时会阻塞，保证拥有锁的线程可以安全的执行临界区内的代码，不用担心线程上下文切换
...
- 线程不安全性能好，假如开发中不会存在多线程安全问题，建议使用线程不安全的设计类
  ...
  ```
  synchronized(对象)类似于在对象中加上锁，当有线程执行到该代码时，相当于拿到对象中的锁，并执行临界区的代码，当其他线程到来时 会因为获取不到锁而被阻塞，一直到有锁线程执行完临界区代码后释放锁才能再次拿到锁执行代码，即使之前拿到锁的线程因为时间片用完而阻塞时也不会释放锁而是继续等待到运行态继续执行完成代码才释放锁，对象相当于锁，synchronized 代码快相当于带上一种锁的房间。
...
> synchronized 只能加在代码块上和方法上 方法可以分为静态方法和实例方法

- 锁对象建议使用共享资源
- 在实例方法中使用 this 作为锁对象，锁住的 this 正好是共享资源
- 在静态方法中使用类名 .class 字节码作为锁对象，因为静态成员属于类，被所有实例对象共享，所以需要锁住类

> 1. synchronized 加在方法上 锁住的是当前实例对象（称之为调用该方法的对象）当多个线程同时访问同一个对象的同一个 synchronized 方法 需要按照顺序执行
> 2. 当其加在类上 锁住的是整个类对象，多个线程同时访问同一个类的同一个 synchronized 静态方法，那么它们也将按顺序排队执行

同步代码块格式：

```java
// 锁对象可以是任意对象 当线程访问锁对象时 其他线程在访问会阻塞 this表示线程的当前实例
synchronized(锁对象){
	// 访问共享资源的核心代码
}
```
...
- 局部变量是线程安全的
- 局部变量引用的对象不一定线程安全（逃逸分析：局部变量是否被其他线程引用）：
  - 如果该对象没有逃离方法的作用访问，它是线程安全的（每一个方法有一个栈帧）
  - 如果该对象逃离方法的作用范围（使用 public 让子类修改），需要考虑线程安全问题（暴露引用）

![asset_img](Prog/2023-05-25-15-50-23.png)
...
- **每个方法是原子的，但多个方法的组合不是原子的**，只能保证调用的方法内部安全：
  ...
  // 线程1，线程2 get和put方法会受到线程上下文切换的影响
  if(table.get("key") == null) {
  	table.put("key", value);
  }
  //当多个线程进行get判断后 可以出现同时为空的情况 则同时put
  ```
...
- replace,subString 等方法底层是新建一个对象，复制过去
...
存储对象的元数据，用于 JVM 的垃圾回收 对象分配 线程同步等
java 对象头组部分：
1.mark Word 2.指向类的指正 3.数组长度（只有数组对象才有）

##### Monitor 锁

> java 中只能通过 Synchronized 获取对 monitor 的控制权

Monitor 被翻译为监视器或管程

每个 Java 对象都可以关联一个 Monitor 对象，Monitor 也是 class，其**实例存储在堆中**，如果使用 synchronized 给对象上锁（重量级）之后，该对象头的 Mark Word 中就被设置指向 Monitor 对象的指针，这就是重量级
通过 mark Word 可以知道对象的类型

> Mark Word 的主要作用如下：
> 存储对象的哈希码：用于支持快速的哈希表查找等操作。
> 存储对象的分代年龄：用于支持分代垃圾回收算法。
> 存储对象的锁状态：用于支持多线程同步操作。
> 存储对象是否被标记为可回收：用于标识对象是否可以被垃圾回收。
> 存储对象所属的线程 ID：用于辅助实现偏向锁优化。
> 不同锁的 Mark Word 不同

- Mark Word 结构：最后两位是**锁标志位**
  ![asset_img](Prog/2023-06-06-15-14-50.png)

  > 其中无锁和偏向锁的标志位都是 01 只在标志位前面 1bit 作为区分
...
- 当 Thread-2 执行 synchronized(obj) 就会将 Monitor 的所有者 Owner（锁的拥有者） 置为 Thread-2，Monitor 中只能有一个 Owner，**obj 对象的 Mark Word 指向 Monitor**，把**对象原有的 MarkWord 存入线程栈中的锁记录**中（轻量级锁部分详解）
  <img src="https://seazean.oss-cn-beijing.aliyuncs.com/img/Java/JUC-Monitor工作原理1.png" style="zoom:67%;" />
- 在 Thread-2 上锁的过程，Thread-3、Thread-4、Thread-5 也执行 synchronized(obj)，就会进入 EntryList BLOCKED（双向链表）的阻塞队列中
...
轻量级锁是 JVM 自动优化的 语法依旧是 synchronized
...
- 如果 CAS 失败，有两种情况：
  ...
  - 如果是其它线程已经持有了该 Object 的轻量级锁（锁标志位不是 01），这时表明有竞争，进入锁膨胀过程
...
- 当 Thread-0 退出同步块解锁时，使用 CAS 将 Mark Word 的值恢复给对象头失败，这时进入重量级解锁流程，即按照 Monitor 地址找到 Monitor 对象，设置 Owner 为 null，唤醒 EntryList 中 BLOCKED 线程
  > 还原时候 Mark Word 地址已经改为了重量锁的地址 所以还原失败
...
偏向锁的思想是偏向于让第一个获取锁对象的线程，这个线程之后重新获取该锁不再需要同步操作（相当于对锁进行偏护） 用于消除线程锁重入带来的 CAS 开销：

> CAS 操作是一种多线程同步操作 用于解决并发访问共享资源时的竞态条件问题
...
- 如果开启了偏向锁（默认开启 state 为 Biased），那么对象创建后，MarkWord 值为 0x05 即最后 3 位为 101，thread、epoch、age 都为 0
...
- 当没有开启偏向锁 那么对象创建后 MarkWord 最后三位为 001 这时他的 hashcode 和 age 都为 0 等到第一次用到 hashcode 才赋值
...
对相同对象多次加锁，导致线程发生多次重入，频繁的加锁操作就会导致性能损耗，可以使用锁粗化方式优化, 将多次锁的请求合并为一个请求 降低短时间内的大量锁请求的性能消耗。
...
需要获取对象锁后才可以调用 `锁对象.wait()`，notify **随机**唤醒一个线程，notifyAll 唤醒所有线程去竞争 CPU

Object 类 API：

```java
public final void notify():唤醒正在waitset等待对象监视器的单个线程。
public final void notifyAll():唤醒正在waitset等待对象监视器的所有线程。
public final void wait():导致当前线程等待，直到另一个线程调用该对象的 notify() 方法或 notifyAll()方法。
public final native void wait(long timeout):有时限的等待, 到n毫秒后结束等待，或是被唤醒
```

> wait 和 notify 都必须是对获得了锁后的线程进入 waitset 队列中的线程进行操作
...
- Owner 线程发现条件不满足，调用 wait 方法，即可进入 WaitSet 变为 WAITING 状态(已经获得锁之后放弃锁的状态)
- BLOCKED（没有获得锁的线程） 和 WAITING 的线程都处于阻塞状态，不占用 CPU 时间片
...
虚假唤醒：notify 只能**随机**唤醒一个 WaitSet 中的线程，这时如果有其它线程也在等待，那么就可能唤醒不了正确的线程

解决方法：采用 notifyAll

notifyAll 仅解决某个线程的唤醒问题，使用 if + wait 判断仅有一次机会，一旦条件不成立，无法重新判断

解决方法：用 while + wait，当条件不成立，再次 wait 防止虚假唤醒
...
Java 内存模型是 Java Memory Model（JMM），本身是一种**抽象的概念**，实际上并不存在，描述的是一组规则或规范，通过这组规范**定义了程序中各个变量（包括实例字段，静态字段和构成数组对象的元素）的访问方式**
...
线程对变量的所有操作都在各自的工作内存中进行，各个线程的内存变量相互独立，工作内存变量之间的交互借助主内存进行交换。
...
1. 一个新的变量只能在主内存中诞生，不允许在工作内存中直接使用一个未被初始化（assign 或者 load）的变量，即对一个变量实施 use 和 store 操作之前，必须先自行 assign 和 load 操作
1. 一个变量在同一时刻只允许一条线程对其进行 lock 操作，但 lock 操作可以被同一线程重复执行多次，多次执行 lock 后，只有**执行相同次数的 unlock** 操作，变量才会被解锁，**lock 和 unlock 必须成对出现**
1. 如果对一个变量执行 lock 操作，将会**清空工作内存中此变量的值**，在执行引擎使用这个变量之前需要重新从主存加载
1. 如果一个变量事先没有被 lock 操作锁定，则不允许执行 unlock 操作，也不允许去 unlock 一个被其他线程锁定的变量
1. 对一个变量执行 unlock 操作之前，必须**先把此变量同步到主内存**中（执行 store 和 write 操作）
