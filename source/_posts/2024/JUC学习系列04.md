...
- NEW 新建状态 没有调用 start()
- RUNNABLE 可运行(分为等待 cpu 时间片或者拿到了 cpu 资源对应于操作系统的就绪和运行态)
- BLOCKED 阻塞（等待获取锁资源）
- WAITING 无限等待（等待其它线程显式地唤醒，否则不会被分配 CPU 时间片），有限等待（超时自动唤醒）
- TERMINATED 终止
...
进程：进程是代码在数据集合上的一次运行活动，是系统进行资源分配和调度的基本单位。理解为系统中的应用程序
线程：线程是进程的一个执行路径，一个进程中至少有一个线程，多个线程共享进程资源。理解为应用程序中的子内容 就像下载软件中的子线程下载任务
